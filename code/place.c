 ...
 */
		...
		default: error("place_string: bad side_of primary terminal", m->name);
	    }
	    /* update all the terminal postions and sides */
...
/*	    if( yfloor < 0) */
	    fixup_string(i, xfloor, yfloor - 1);    
    ...
    int part;
{
	...
	fixup_partition(part, xfloor, yfloor);	
    }
}
    ...
    FILE *f;				/* The file to dump it to (Better be open!) */
{
	...
	fprintf(f, "\n(%s: Module Placement for %s) %d %d %d %d init\n",
                getenv("USER"), ivffilename, xfloor, yfloor, x_sizes[0], y_sizes[0]);
    }
    else 
    {
	ps_print_standard_header(f);
