 ...
 *----------------------------------------------------------------------
 * File 	psfigs.c
 * Author 	s.t.frezza (frezza@jupiter.ee.pitt.edu)
 * 
...
int ps_print_sym(f, sym, x, y, x_dis, y_dis, rot, name)
    FILE *f;
    char *sym, *name;
    int x, y, x_dis, y_dis, rot;
{
    ...
    switch(validate(sym))
    {
	case BUFFER : ps_print_buffer(f, x, y, x_dis, y_dis);
	              ps_put_label(f, name, (float)x + (float)x_dis/2. - (float)strlen(name)/2.0,
				   (float)y + 2.25);
		      break;
	case INVNOT_  : ps_print_inverted_not(f, x, y, x_dis, y_dis);
	            ps_put_label(f, name, (float)x + (float)x_dis/2. - (float)strlen(name)/2.0 + 1.0,
				 (float)y + 2.25);
		    break;
	case NOT_  : ps_print_not(f, x, y, x_dis, y_dis);
	            ps_put_label(f, name, (float)x + (float)x_dis/2. - (float)strlen(name)/2.0,
				 (float)y + 2.25);
		    break;
	case AND  : ps_print_and(f, x, y, x_dis, y_dis);
	            ps_put_label(f, name, 
				 (float)x + (float)x_dis/2. - (float)strlen(name)/2.0 - .2,
				 (float)y + (float)y_dis/2. - NAME_OFFSET);
	            break;
	case NAND : ps_print_nand(f, x, y, x_dis, y_dis);
	            ps_put_label(f, name, 
				 (float)x + (float)x_dis/2. - (float)strlen(name)/2.0 - .2,
				 (float)y + (float)y_dis/2. - NAME_OFFSET);
	            break;
	...
	case XOR  : ps_print_xor(f, x, y, x_dis, y_dis);
	            ps_put_label(f, name, 
				 ...
				 (float)y + (float)y_dis/2. - NAME_OFFSET);
	            break;
	case XNOR : ps_print_xnor(f, x, y, x_dis, y_dis);
	            ps_put_label(f, name, 
				 (float)x + (float)x_dis/2. - (float)strlen(name)/2.0 - .2,
				 (float)y + (float)y_dis/2. - NAME_OFFSET);
	            break;

	case INPUT_SYM : ps_print_interm(f, x, y, x_dis, y_dis);
	                 ps_put_label(f, name, 
				      (float)x + (float)x_dis/2. - (float)strlen(name)/2.0 - 3.2,
				      (float)y + (float)y_dis/1.5 - NAME_OFFSET);
	                 break;
	case OUTPUT_SYM : ps_print_outterm(f, x, y, x_dis, y_dis);
	                  ps_put_label(f, name, 
				       (float)x + (float)x_dis/2. - (float)strlen(name)/2.0 + .1,
				       (float)y + (float)y_dis/2. - NAME_OFFSET);
	                  break;
	 
        case BLOCK : ps_print_block(f, x, y, x_dis, y_dis, rot);
...
int validate(s)
    char *s;
{			
    /* SEE "ivf_format.h" and "network.h" */
