...
static int index = 0;		/* Used for printing edge visitation order */
    ...
    int refX, refY;
{
    ...
    arc *Carc = (arc *)curTile->this, *Parc = (arc *)prevTile->this;
    ...
    net *n;
{
	...
	*/
	 ...
	 {
	     thisTile = tl->this;
	     ar = (arc *)thisTile->this;		lar = (arc *)lasTile->this;
	     
	     /* Standard Linkage control: */
	     if ( ((arc *)(thisTile->this))->page == VERT )
	     {
    ...
    int stopAt;
{
    ...
    for (rl = orderedRanges->next; ((rl != NULL) && (index++ < stopAt)); rl = rl->next)
    {
	n++;
	max = rl->this->sr->q2;
	...
	{					/* constraint is found */
		...
		fixup = rev_decoupleable_p(orderedRanges, index);
...
/*------------------------------------------------------------------------------------*/
int help_setup_dependence_levels(index, level, dChains)
    int index, level;
    rnglist **dChains;		/* Deeply recursive */
{
    /* Find the maximum dependence level from this set of chains */
    ...
    tilist *tl;
{
    ...
    {
	/* Walk down this ordered list of arcs, resolving the overlapped ranges */
	a = (arc *)tll->this->this;
...
find_and_merge_slivers(a, n)
    ...
    net *n;
{
    /* Look to see if there are other adjacent arcs/tiles that can be used.
       If so, then modify the corner limits, and do the book keeping for the 
       arc/corner structures...*/
...
list *extract_corner_list(f, n)
    ...
    net *n;
{
    /* This picks loops through the corners from <n>->route, and builds a
       list of point lists (corners) that are to be printed. */
...
/*	fprintf(f,"\n(N2A %s %s input:%s s:%d c:%d r:%-5.3f rule #%d part:%d dir:%d) %d %d %d %d init\n", 
	getenv("USER"), ctime(&time_loc),ivffilename, max_partition_size,
    ...
    FILE *f1, *f2;
{
   ...
   /* Define the bounding box for the post-script code: */
	...
	fprintf(f1,
                "\n(Horizontal Tilespace for N2A %s %s input:%s s:%d rule #%d part:%d dir:%d) %d %d %d %d init\n",
                getenv("USER"), ctime(&time_loc),
                ivffilename, max_partition_size, partition_rule,
                partition_count, matrix_type,
                xfloor, yfloor, x_sizes[0], y_sizes[0]);

	ps_print_standard_header(f2);
	fprintf(f2,
                "\n(Vertical Tilespace for N2A %s %s input:%s s:%d rule #%d part:%d dir:%d) %d %d %d %d init\n",
                getenv("USER"), ctime(&time_loc),
                ivffilename, max_partition_size, partition_rule,
                partition_count, matrix_type,
                xfloor, yfloor, x_sizes[0], y_sizes[0]);
	...
	fprintf(f2, "    180 rotate %d neg %d neg tanslate \n", 
...
void quality_statistics()
{
    ...
    fprintf(f,"| %s -p%d -s%d -c%d -r%-4.2f -m%d \t ||",
    ...
    int page;
{
    ...
    fprintf(qsf,"%s -p%d -s%d -c%d -r%-4.2f %s %s %s\t ||",
	    ivffilename, partition_rule, max_partition_size, max_partition_conn,
