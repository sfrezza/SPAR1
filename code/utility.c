    ...
    char *gate_name, *gate_type;
{
    ...
    object->dly_type = INERTIAL_DELAY;
    object->delay = 0;
    object->x_pos = 0;
    object->y_pos = 0;    
    object->flag = UNSEEN;
    object->placed = UNPLACED;
    object->rot = LEFT;		/* asuume no rotation */
...
/* OLD:tt->y_pos = ((index % 2) == 1) ? tt->mod->y_size/2 - index/2	         */
/*	                              : tt->mod->y_size/2 +  index/2 + 1;        */
...
/* ------------------------------------------------------------------------
 * reset_default_icon_size - use the x,y dimensions from .ivf   added 3-92 (stf)
 *-------------------------------------------------------------------------
 */
    ...
    int incount, inoutcount, outcount;
{
    ...
    if((generate_sced == TRUE) &&
       (!read_terms(cur_module->type, incount, outcount, cur_module)))
    {	
	/* punt & pray */
    }

    else if (compute_aspect_ratios == TRUE) 
    {
...
/* ------------------------------------------------------------------------
 * addXCpositions: add the real postions for the terminals, as set by the
 * object library file defined in WCircuit, if we can find it. Otherwise use the 
 * defaults that were done above in add_in_out. INCOMPLETE
 * ------------------------------------------------------------------------
 */
void addXCpositions(cur_module, incount, inoutcount, outcount, inoutcount)
    module *cur_module;
    int incount, inoutcount, outcount;
{
    tlist *t;
...
/* ------------------------------------------------------------------------ */
int get_4_terminal_counts(m, ins, inouts, outs, outins)
    module *m;
    int *ins, *inouts, *outs, *outins;
{
    tlist *tl;
    for (tl = m->terms; tl != NULL; tl = tl->next)
    {
...
/* ------------------------------------------------------------------------
 * adddelay: add the simulation delay information to a module
 * ------------------------------------------------------------------------
 */
void adddelay(time)
    int time;
{
    curobj_module->delay = time;
}
