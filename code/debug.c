    ...
    FILE *f;
{
    ...
    {
	fprintf(f, "\nModule index: %d name: %s,\ttype: %s\n",
		m->this->index, m->this->name, m->this->type);
	fprintf(f, "\tdelay_type: %d,\tdelay_value: %d,\tflag: %d\n",
		m->this->dly_type, m->this->delay, m->this->flag);
...
void print_distance_matricies()
{
    ...
    fprintf(mfile,"- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - \n");
    fprintf(mfile,"| %s -p%d -s%d -c%d -r%-4.2f -m%d %s %s %s\t ||",
...
{
    fprintf(stderr, "Active list: %s\n", s);
    
    for (; al != NULL; al = al->next)
    {
	fprintf(stderr, "wave:%d\t crosses:%d\t prev:%lx\n",
		al->this->b, al->this->c, (long)al->this->prev);
	fprintf(stderr, "\tindex:%d\t x-low:%d\t y-high:%d\t dir:%s\t type:%s\n",
		al->this->i, al->this->x, al->this->y,
		(al->this->d == UP)? "UP":
		(al->this->d == DOWN)? "DOWN":
		(al->this->d == LEFT)? "LEFT": "RIGHT",
		(al->this->type == MODULE)? "MODULE":
		(al->this->type == NET)? "NET":
		(al->this->type == ACTIVE_A)? "ACTIVE_A":"ACTIVE_B"
		);
    }
}
...
print_ends(el)
    elist *el;
{
    fprintf(stderr, "Ends list\n");
    
    for (; el != NULL; el = el->next)
    {
	fprintf(stderr, "\tindex:%d\t x-low:%d\t y-high:%d\t type:%s\n",
		el->this->i, el->this->x, el->this->y,
		(el->this->type == SEG)? "SEG":
		(el->this->type == LEFT_C)? "LEFT_C": "RIGHT_C"
		);
    }
}
    ...
    FILE *f;
{
	...
	fprintf(f,
		...
		ivffilename, max_partition_size, max_partition_conn, partition_ratio, partition_rule,
...
void draft_statistics()
{
    ...
    fprintf(f,"%s -p%d -s%d -c%d -r%-4.2f | %d\t ||",
	    ivffilename, partition_rule, max_partition_size, max_partition_conn,
