 ...
 * rip-up if they happen to be in using the most congested arc on along a net that is 
...
int contents_equivalentp_for_arcs(t1, t2)
    tile *t1, *t2;
{
    arc *a1 = (arc *)t1->this, *a2 = (arc *)t2->this;
...
distribute_corner_list(owner, partner)
    arc *owner, *partner;	/* <owner> contains <corners>, <partner>s ->corners
				   should be NULL. */
{
    ...
    list *tilePairList;
{
	...
	/* Create the arc for the <child> arc and tie it in: */
	pArc = (arc *)parent->this;	
	oldArc = (arc *)child->this;
...
{	/* This tile was just created... */
    arc *q = create_arc(new, currentPage, TRUE);
}
/*------------------------------------------------------------------------------------*/
    ...
    tile *child, *parent;
{
    ...
    arc *cArc, *pArc = (arc *)parent->this, *oldArc = (arc *)child->this;
    ...
    trail *t;
{
    ...
    {
	remove_trailA(t, (arc *)tl->this->this);   /* Pull from arc hash table */
    }
    free_list(t->used);
}
/*------------------------------------------------------------------------------------*/
    ...
    trail *tOld, *tNew;
{
    ...
    {
	remove_trailA(tOld, (arc *)tl->this->this);   /* Pull from arc hash table */
	insert_trailA(tNew, (arc *)tl->this->this);
    }
    ...
    tile *ti;
{
	...
	if (trl->this->ex == ex)	/* Only look at trails belonging to <ex> */
	{
		...
		pop(&t->tr);			       /* Yank this (extraneous) tile.*/
            ...
            if (tl != NULL)
	    {	/* Now remove <ti>: */
		remove_trailA(t, (arc *)tl->this->this );        
...
int congestion_cost(a)
    arc *a;		/* arc to be used... */
{
    /* This is an estimate of what it costs to use the given arc.  This should be
    ...
    trail *t;
{
    ...
    add_trail_copy(newTrail);				/* Add to all arc hash tables */
...
{
    /* Record the termination position (arc) of the expansion, notify the net that 
...
int meets_termination_conditions_p(tr, parentEx, besTrail)
    ...
    trail *besTrail;		/* Best trail so far in the given arc */
{
    ...
    expnlist **expnsStillActive;
{
    ...
    expn *parentEx = t->ex;
    arc *a = (arc *)t->tr->this->this;		  /* Proposed joint arc for <parent> */
...
{	/* Generalized form of "examine_best_trailE" */
    ...
    for (exl = ex->siblings; exl != NULL; exl = exl->next)
    {
	/* Check <ex> and all of <ex>'s siblings for te best trail... */
...
/*------------------------------------------------------------------------------------*/
expn *make_next_best_move(ex, trailsCompleted)
    expn *ex;
    ...
    int side, x_pos, y_pos, page;
{
	...
	if (hTile->y_pos + hTile->y_len == y_pos)
	{
    ...
    expnlist **expnsToMove;
{
    ...
    expn *jEx;
    expnlist *exl, *terminationCandidates;
    trail *t, *temp;
    trlist *trl, *trailsCompleted = NULL;
	...
	{
	    make_next_best_move(exl->this, &trailsCompleted);
	}

	/* check all expansion groups for terminations: */
	for (trl = (trlist *)sort_list(trailsCompleted, trail_cost_order);
	     trl != NULL; trl = trl->next)
	{	    

	    t = trl->this;

	    terminationCandidates = check_for_termination(t);
	    for(exl = terminationCandidates; exl != NULL; exl = exl->next) 
	    {
...
int filled_p(a)
    arc *a;
{
    /* returns TRUE if the given arc is filled */
...
reset_all_trail_costs(nets, part)
    ...
    int part;		/* Not used */
{
    /* This resets the trail costs for the entire search space. */
    arc *ar;
    nlist *nl;
    expnlist *exl;
    expn *ex;
...
{	
	...
	mod = mll->this;
	tarc = (arc *)mod->htile->this;	     /* As set in 1st loop in "map_into_arcs" */
    ...
    FILE *f;
{
   ...
   /* Define the bounding box for the post-script code: */
	...
	fprintf(f, "\n(N2A %s %s %s %s)\n",
                getenv("USER"), ctime(&time_loc), ivffilename, 
